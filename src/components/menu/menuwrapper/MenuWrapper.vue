<template>
    <div class="menu-wrapper" ref="menu">
        <div @click="triggerClick" class="menu-button-container" ref="buttonContainer">
            <slot name="menu-button" v-if="$slots['menu-button']" :toggleMenu="(e) => toggleMenu(e)">

            </slot>
            <button v-else class="menu-wrapper-button" ref="button" :aria-expanded="isOpen.toString()"
                aria-haspopup="true" :aria-controls="id" @click="toggleMenu">
                <slot name="menu-button-text"></slot>
            </button>
        </div>
        <teleport to="body">
            <ul class="menu-wrapper-menu" :class="classRoot" v-if="isOpen" ref="list" role="menu" :id="id"
                @close_menu="closeHandler()" :style="{
                    left,
                    right,
                    top,
                    bottom,
...
<script>
import { nextTick, ref, onMounted, onUnmounted, onBeforeUnmount, watch, getCurrentInstance } from 'vue';
...
export default {
    ...
    setup(props, { emit }) {
        ...
        function isDispose(target) {
            let el = proxy.$refs.list;
            const classes = [];
            const dispose = classes.findIndex((className) => {
                if (target.closet && (target.closet(className) || target.className.contains(className))) {
                    return true;
                }
                return false;
            }) === -1;
